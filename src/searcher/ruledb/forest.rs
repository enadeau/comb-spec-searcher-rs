    ...
    impl Function {
        ...
        pub fn preimage_gap(&self, gap_size: u32) -> u32 {
            ...
            for (i, v) in self.preimage_count.iter().enumerate() {
                dbg!(i, i as u32, v, last_non_zero, gap_size);
                if *v != 0 {
                    dbg!("y");
                    last_non_zero = i as u32;
                } else if i as u32 - last_non_zero >= gap_size {
                    dbg!(i as u32 - last_non_zero);
                    dbg!("yy");
        ...
        ///Increase the value vector to include the last input
        fn increase_value_vector(&mut self, last_input: u32) {
            let num_new_entry = last_input - self.values.len() as u32 + 1;
            self.values
                .resize(last_input as usize + 1, IntOrInf::Int(0));
            self.preimage_count[0] += num_new_entry;
        }
    }

    struct FunctionPreImageIterator<'a> {
        function: &'a Function,
        value: IntOrInf,
        pos: u32,
    }
